...
在Vue.js中只有data中已经存在的数据才会被Observe为响应式数据，如果你是新增的属性是不会成为响应式数据的，因此Vue提供了一个api(Vue.$set())来解决这个问题。
...
1. 判断类型
2. target为数组调用splice方法
3. target为对象，且key不是原型对象上的属性，直接修改
4. target不能是Vue实例，或者Vue实例的根数据对象，否则报错
5. target是非响应式数据时，我们就按照普通对象添加属性的方式来处理
6. target为响应数据，且**key为新增属性**，我们key设置为响应式，并手动触发其属性值的更新
...
defineReactive方法进行响应式处理
 - defineReactive方法就是Vue在初始化对象时，给对象属性采用 Object.defineProterty动态添加getter和setter的功能所调用的方法
